...
// mathjs replaced with native Math for performance
...
const debounce = <T extends (...args: unknown[]) => unknown>(fn: T, wait: number) => {
...
const useResponsiveDimension = (responsive: boolean, config: GradualBlurConfig, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v = config[key];
      if (w <= 480) v = config[key];
      else if (w <= 768) v = config[key];
      else if (w <= 1024) v = config[key];
      setValue(v);
    };
